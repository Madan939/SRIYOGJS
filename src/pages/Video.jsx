...
// Lazy Youtube Thumbnail Component
const LazyYoutube = ({ videoId, caption }) => {
  const [loaded, setLoaded] = useState(false);

  return (
    <div className="w-full mb-4 relative rounded-xl overflow-hidden shadow-xl aspect-video">
      {!loaded ? (
        <div
  ...
  return (
    <div className="w-full">
      ...
      <section className="mb-[56px] bg-[#8b181b] w-full flex justify-center items-center py-10">
        <section className="w-[1140px] max-lg:w-[88%] max-md:w-[89%]">
          <div className="mb-2">
            <h1 className="text-[35px] justify-center items-center flex mt-2 font-semibold text-white">
              {name}
            </h1>
          </div>
          <div className="w-full aspect-video">
      ...
      <section className="w-[1140px] max-lg:w-[88%] max-md:w-[89%] mx-auto px-4 py-10">
        ...
        <div className="grid gap-12 grid-cols-1 sm:grid-cols-2">
          {videos.map((video, index) => {
            ...
            return (
              <div key={index}>
                <LazyYoutube videoId={videoId} caption={video.caption} />
                <p className="sm:text-[18px]">{video.caption}</p>
              </div>
            );
          })}
        </div>
